...
describe('help', () => {
  it('is opened from settings and does not pretend feedback was sent', () => {
    ...
    const howTo = help.slice(help.indexOf('export function HelpBody'), help.indexOf('export function FeedbackDialog'))
    expect(howTo).toContain('Take off')
    expect(howTo).toContain('Calibrate from the drawing')
    expect(howTo).not.toContain('textarea')
    expect(howTo).not.toContain('What happened')
    expect(howTo).not.toContain('help-feedback')
    expect(help).toContain('Worker-side Notion token')
    expect(help).toContain('createPortal')
    expect(help).toContain('role="dialog"')
    expect(help).toContain('className="feedback-card"')
